...
class AdaptiveEngine:
    ...
    def apply_learning(self, events: Iterable[RiskEvent]) -> AdaptiveUpdateResult:
        ...
        if events_list:
            self.last_learning_update = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    def receive_threat_packet(self, packet: ThreatPacket) -> None:
        ...
        self.last_threat_received = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
